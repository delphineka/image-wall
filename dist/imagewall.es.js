  ...
  async layout() {
    ...
    for (const t of L) {
      ...
      for (let p = 0; p < t.items.length; p++) {
        ...
        w.style.position = "absolute", w.style.left = `${m}px`, w.style.top = `${B}px`, w.style.width = `${I}px`, w.style.height = `${E}px`, w.style.margin = "0", m += I + u;
      }
      B += t.height + u;
    }
    this.container.style.height = `${Math.max(0, B - u)}px`, this.dispatchEvent(new Event("layout")), this.opts.debug && console.debug("[ImageWall] layout complete, total height:", this.container.style.height);
  }
